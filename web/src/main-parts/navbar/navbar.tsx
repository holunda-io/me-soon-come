...
class Navbar extends Component<any, any> {
  ...
  handleCreateNewRequest = (event: any) => {
    ...
    axios.post('http://localhost:8080/request').then(res => {
      console.log('Created new request with id x: ', res);
    })
  };
    ...
    return (
      <AppBar position="sticky">
        <Toolbar>
          ...
          {auth && (
            <div>
              <Tooltip title="create new request" aria-label="create new request">
                <IconButton
                  aria-owns={auth ? 'menu-appbar' : undefined}
                  className={classes.menuButton}
                  aria-haspopup="true"
                  onClick={this.handleCreateNewRequest}
                  color="inherit"
                >
