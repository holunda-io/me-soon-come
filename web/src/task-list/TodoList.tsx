...
class TodoList extends React.Component<ITaskListProps, ITaskListState> {
  ...
  componentDidMount() {
    ...
    this.props.service.getTodoListChannel().subscribe("add", ((event) => {
    ...
    this.props.service.getTodoListChannel().subscribe("remove", ((event) => {
      const { taskEvents } = this.state;
      this.setState({
        tasksAvailable: true,
    ...
    const taskList = (
      <List dense className={classes.root}>
        ...
        {this.state.taskEvents ? this.state.taskEvents.map(taskEvent => {
          ...
          return <TodoListItem taskEvent={taskEvent} key={"task_list_item" + taskEvent.id} />
        }) : <EmptyStateTaskList />}
